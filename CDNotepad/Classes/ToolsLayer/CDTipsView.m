...
@interface CDTipsView()
{
    ...
    UIView *_targetView; //  需要显示这个tip的view；
    UIView *_contentView;
    UILabel *_text;
...
- (void)initContentView
{
    if (_contentView) {
        ...
        if (_backgroundColor) {
            ...
            _contentView.backgroundColor = DefineColorRGB(0, 0, 0, 0.8);
        }
        ...
        _fontSize = _fontSize > 0 ? _fontSize : 10.0;  //  默认的吐司字体大小为12
    }
}
...
- (void)initSubviewLayer
{
    ...
    _contentView.bounds = CGRectMake(0, 0, sizeOfText.width + INSERT_W+5, sizeOfText.height + INSERT_H+5);
...
{
    [self hiden:animation complete:^(BOOL animation) {
        self.frame = _targetView.bounds;
        [_targetView addSubview:self];
        [self layoutIfNeeded];  //  回复原始位置
...
- (void)hiden:(BOOL)animation complete:(void(^)(BOOL animation))complete
{
...
{
    [self hiden:animation complete:nil];
}
